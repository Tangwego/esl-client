...
public class EslEvent {
    ...
    private void parsePlainBody(final List<String> rawBodyLines) {
        System.out.println("parsePlainBody:");
        System.out.println("***************************");
    ...
    private void parseJsonBody(List<String> rawBodyLines) {
        for (String rawLine : rawBodyLines) {
            JSONObject jsonObject = JSONObject.parseObject(rawLine);
            for (Map.Entry entry : jsonObject.entrySet()) {
                String headerName = entry.getKey().toString();
                String headerValue = entry.getValue().toString();
                log.trace("Name:{}, Value:{}", headerName, headerValue);
                if (decodeEventHeaders) {
                    try {
                        if (headerName.equals("_body")) {
                            String body = jsonObject.get("_body").toString();
                            if (body.length() > 0) eventBody.add(URLDecoder.decode(body, "UTF-8"));
                        } else {
                            eventHeaders.put(headerName, URLDecoder.decode(headerValue, "UTF-8"));
                        }
                    } catch (UnsupportedEncodingException e) {
                        log.warn("Could not URL decode [{}]", headerValue);
                        eventHeaders.put(headerName, headerValue);
                    }
                } else {
                    if (headerName.equals("_body")) {
                        String body = jsonObject.get("_body").toString();
                        if (body.length() > 0) eventBody.add(body);
                    } else {
                        eventHeaders.put(headerName, headerValue);
                    }
    ...
    public String getEventContent() {
        ...
        sb.append("\"EslEvent\":{\n");
        sb.append("\"Event-Message-Headers\":{\n");
        for(Map.Entry<Name, String> entry : messageHeaders.entrySet()) {
        ...
        sb.append("\"Event-Body\":[\n");
        for(String body: eventBody) {
            sb.append("\"" + body + "\"");
            sb.append(",");
        }
        if(sb.charAt(sb.length() - 1) == ','){
